# Pictured below is a matrix. Recreate this matrix in at least three different ways. 

#Player  Piece   
#[1,] "dark"  "king"  
#[2,] "dark"  "queen" 
#[3,] "dark"  "pawn"  
#[4,] "dark"  "pawn"  
#[5,] "dark"  "knight"
#[6,] "light" "bishop"
#[7,] "light" "king"  
#[8,] "light" "rook"  
#[9,] "light" "pawn"  
#[10,] "light" "pawn"  


# Hint. Use the dim() function, the matrix() function, and cbind().
#1.
x <- c('dark' , 'dark' , 'dark','dark' , 'dark', 'light', 'light', 'light', 'light','light','king', 'queen', 'pawm', 'pawn', 'knight', 'bishop' , 'king', 'rook', 'pawn', 'pawn')
player <- x
dim(player) <- c(10,2)
colnames(player) <- c('Player', 'Piece')
print(player)
player <- matrix(x, nrow = 10 , ncol= 2 )
print(player)

x <- c('dark','dark','dark','dark','dark','light','light','light','light','light')
y <- c('king', 'queen', 'pawn', 'pawn', 'knight', 'bishop', 'king', 'rook', 'pawn', 'pawn')
player <- cbind(x , y)
print(player)
# Can you think of a faster way to create this matrix?
  
# Assign names to it in two different ways.

# Would using rbind() be practical in this case?
  
# Can you think of a way to make adding a row to the matrix worthwhile?